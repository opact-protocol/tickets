...
export function createCode(nInputs) {
  if (nInputs < 1 || nInputs > 8)
    throw new Error("Invalid number of inputs. Must be 1<=nInputs<=8");
  const t = nInputs + 1;
  ...
  C.push(
    ethers.utils
      .keccak256(ethers.utils.toUtf8Bytes(`poseidon(uint256[${nInputs}])`))
      .slice(0, 10)
  ); // poseidon(uint256[n])
  C.eq();
  C.swap(1);
  C.push(
    ethers.utils
      .keccak256(ethers.utils.toUtf8Bytes(`poseidon(bytes32[${nInputs}])`))
      .slice(0, 10)
  ); // poseidon(bytes32[n])
  C.eq();
  C.or();
