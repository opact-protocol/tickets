...
contract PlonkVerifier {
    ...
    // pointers to be used in calculations
    // will override proof during execution
    ...
    uint16 constant resultMem = 1704;
    ...
    function verifyProof(bytes memory proof, uint[] memory pubSignals) public view returns (bool) {
        assembly {
            ...
            function calculateChallanges(pProof, pMem, pPublic) {
                ...
                // JASSO:
                // Hashes all previously stored values in sequence
                // publics | A | B | C
                b := mod(keccak256(add(pMem, lastMem), 224), q) 

                // JASSO:
                // Adds values to beta gamma and alpha in system
                // variables
                // overrides proof values that are already stored after lastMem
            ...
            function calculateLagrange(pMem) {
                ...
                let w := 1                
                // LOOP OVER EACH PUBLIC INPUT
                // CREATE ONE EVAL VAR FOR EACH INPUT
                mstore(
                    add(pMem, pEval_l1), 
                    mulmod(
                        n, 
    ...
    function partialProof(bytes memory proof, uint[] memory pubSignals) public view returns (uint[] memory result) {
        assembly {
            /////////
            // Computes the inverse using the extended euclidean algorithm
            /////////
            ...
            function returnValues(pMem) -> values {
            ...
            result := sub(pMem, 0x20)
            mstore(add(result, 0x00), 22)
