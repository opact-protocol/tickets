...
__export(src_exports, {
  HideyourCash: () => HideyourCash,
  MerkleTreeService: () => MerkleTreeService,
  MimcSponge: () => MimcSponge,
  checkWithdrawStorages: () => checkWithdrawStorages,
  cleanupRawAmount: () => cleanupRawAmount,
  createSnarkProof: () => createSnarkProof,
  createTicket: () => createTicket,
  formatBigNumberWithDecimals: () => formatBigNumberWithDecimals,
  getAmount: () => getAmount,
  getDecimals: () => getDecimals,
  getPublicArgs: () => getPublicArgs,
  getRelayerFee: () => getRelayerFee,
  getTokenStorage: () => getTokenStorage,
  getTransaction: () => getTransaction,
  getUTCDate: () => getUTCDate,
  getWithdrawInput: () => getWithdrawInput,
  leInt2Buff: () => leInt2Buff,
  mimc: () => mimc,
  parseNearAmount: () => parseNearAmount,
  parseNote: () => parseNote,
  prepareMerkleTree: () => prepareMerkleTree,
  prepareWithdraw: () => prepareWithdraw,
  randomBN: () => randomBN,
  sendAllowlist: () => sendAllowlist,
  sendContractWithdraw: () => sendContractWithdraw,
  sendDeposit: () => sendDeposit,
  sendJsonRpc: () => sendJsonRpc,
  sendTransactionsCallback: () => sendTransactionsCallback,
  sendWithdraw: () => sendWithdraw,
  shortenAddress: () => shortenAddress,
  trimLeadingZeroes: () => trimLeadingZeroes,
  viewAccountBalance: () => viewAccountBalance,
  viewAccountHash: () => viewAccountHash,
  viewAllCurrencies: () => viewAllCurrencies,
  viewCurrencyContracts: () => viewCurrencyContracts,
  viewFunction: () => viewFunction,
  viewIsAllowlistRootValid: () => viewIsAllowlistRootValid,
  viewIsContractAllowed: () => viewIsContractAllowed,
  viewIsInAllowlist: () => viewIsInAllowlist,
  viewIsWithdrawValid: () => viewIsWithdrawValid,
  viewRelayerHash: () => viewRelayerHash,
  viewWasNullifierSpent: () => viewWasNullifierSpent
});
module.exports = __toCommonJS(src_exports);

// src/services/mimc.ts
var import_circomlibjs = require("circomlibjs");
var MimcSponge = class {
  sponge;
  initialized = false;
  async initMimc() {
    const sponge = await (0, import_circomlibjs.buildMimcSponge)();
    return {
      hash: (left, right) => {
        return sponge.F.toString(
          sponge.multiHash([BigInt(left), BigInt(right)])
        );
      },
      singleHash: (single) => {
        return sponge.F.toString(sponge.multiHash([BigInt(single)]));
      }
    };
  }
};
var mimc = new MimcSponge();

// src/services/merkle-tree.ts
var import_graphql_request = require("graphql-request");
var import_fixed_merkle_tree = require("fixed-merkle-tree");
var MerkleTreeService = class {
  name;
  contract;
  graphqlUrl;
  branchesQuery;
  lastBranchesQuery;
  tree;
  constructor(contract, name, graphqlUrl, branchesQuery, lastBranchesQuery) {
    this.name = name;
  ...
  async initMerkleTree(cache) {
    const { hash } = await mimc.initMimc();
    const items = await this.getBranches(cache);
    const tree = new import_fixed_merkle_tree.MerkleTree(20, [], {
  ...
  async getMerkleTreeBranchesWithQuery({ query }, variables = {}) {
    return (0, import_graphql_request.request)(this.graphqlUrl, query, variables);
  }
  async getMerkleTreeBranchesWithPaginatedQuery({ query }, variables = {}) {
    ...
    for (let i = 0; i < totalPage; i++) {
      const { [this.branchesQuery.name]: data } = await (0, import_graphql_request.request)(
        this.graphqlUrl,
        query,
        {
          ...variables,
...
// src/constants/near.ts
var AttachedGas = "300000000000000";
var OneNear = "1000000000000000000000000";
var nearNominationExp = 24;

// src/actions/connection.ts
var sendTransactionsCallback = async (connection, transactions) => {
  if (connection.functionCall) {
    const outcomes = [];
    for (let i = 0; i < transactions.length; i++) {
      const { receiverId, actions } = transactions[i];
      const { params: { methodName = "", deposit = "", args = {} } = {} } = actions[0] || {};
...
// src/helpers/date.ts
var getUTCDate = (timestamp = Date.now()) => {
...
var _nextId = 321;
var getTransaction = (signerId, receiverId, method, args, amount, skipParser = false) => {
...
var viewFunction = async (nodeUrl, contractId, methodName, args = {}) => {
...
var sendJsonRpc = async (nodeUrl, method, params) => {
  ...
  const rpcService = import_axios.default.create({
    baseURL: nodeUrl,
    headers: {
...
var getAmount = (amount, skip = false) => {
  if (!amount) {
    return "1";
  }
  if (skip) {
...
var cleanupRawAmount = (amount) => {
  return amount.replace(/,/g, "").trim();
};
var trimLeadingZeroes = (value) => {
...
function leInt2Buff(value) {
  return new import_bn.default(value, 16, "le");
}
function randomBN(nbytes = 31) {
  return (0, import_web3_utils.toBN)(leInt2Buff((0, import_randombytes.default)(nbytes)).toString());
}

// src/helpers/number.ts
var import_big = __toESM(require("big.js"));
function getDecimals(decimals) {
  return (0, import_big.default)(10).pow(decimals || 1);
}
var formatBigNumberWithDecimals = (value, decimals) => {
  return new import_big.default(value).div(decimals).toFixed(2);
};

// src/helpers/merkle-tree.ts
var parseNote = (note) => {
...
var shortenAddress = (address, chars = 8) => {
  if (!address) {
    return "";
  }
...
var FT_Storage = "0.5";
var createTicket = async (nodeRpcUrl, contract, accountId, currencyId) => {
  const { hash } = await mimc.initMimc();
  ...
  const note = currencyId.toString() + "-" + secret.toString() + "-" + nullifier.toString() + "-" + accountHash.toString();
  return {
    note,
    hash: secretsHash
  };
};
var sendDeposit = async (nodeUrl, hash, amount, depositContract, accountId, currency, connection) => {
  const transactions = [];
...
var getTokenStorage = async (token, contract, nodeRpcUrl) => {
  try {
...
// src/actions/relayer.ts
var import_axios2 = __toESM(require("axios"));
var getRelayerFee = async (relayer, accountId, instanceId) => {
...
var sendWithdraw = async (relayer, payload) => {
  return (0, import_axios2.default)({
...
var sendContractWithdraw = async (nodeUrl, contract, signerId, receiverId, publicArgs, connection) => {
...
// src/actions/compliance.ts
var sendAllowlist = async (nodeUrl, contract, accountId, connection) => {
  const transactions = [];
  transactions.push(
    getTransaction(
      accountId,
...
// src/actions/snark-proof.ts
var import_snarkjs = require("snarkjs");
var createSnarkProof = async (payload, verifierUrl = "./verifier.wasm", circuitUrl = "./circuit.zkey", logger) => {
  try {
    const { proof, publicSignals } = await import_snarkjs.plonk.fullProve(
      payload,
      verifierUrl,
      circuitUrl,
      logger
...
var prepareWithdraw = async (nodeUrl, contract, fee, note, relayer, recipient, logger, allowlistTree, commitmentsTree, verifierUrl = "./verifier.wasm", circuitUrl = "./circuit.zkey") => {
  const { hash, singleHash } = await mimc.initMimc();
...
var getWithdrawInput = async ({ hash }, fee, recipientHash, path, pathAL, commitment, single_hash) => {
  return {
    fee,
    root: path.pathRoot,
...
var getPublicArgs = (proof, relayer, publicSignals, receiver) => {
  return {
...
// src/actions/merkle-tree.ts
var prepareMerkleTree = async (contract, name, branchesQuery, lastBranchesQuery, graphqlUrl, cache) => {
...
// src/graphql/merkle-tree.ts
var import_graphql_request2 = require("graphql-request");
var allowListUpdatesQuery = {
  name: "allowlistMerkleTreeUpdates",
  query: import_graphql_request2.gql`
...
// src/services/hideyourcash/views.ts
...
// src/services/hideyourcash/actions.ts
var Actions = class extends Views {
  nodeUrl;
  contract;
  graphqlUrl;
  verifierUrl;
  circuitUrl;
  constructor(nodeUrl, contract, graphqlUrl, verifierUrl = "./verifier.wasm", circuitUrl = "./circuit.zkey") {
  ...
  async sendAllowlist(accountId, connection) {
    return sendAllowlist(this.nodeUrl, this.contract, accountId, connection);
  }
  async createTicket(accountId, currencieContract) {
    return createTicket(
      this.nodeUrl,
      this.contract,
      accountId,
  ...
  async sendDeposit(hash, amount, contract, accountId, currency, connection) {
    return sendDeposit(
      this.nodeUrl,
      hash,
      amount,
  ...
  async sendContractWithdraw(contract, signerId, receiverId, publicArgs, connection) {
  ...
  async getRelayerFee(relayer, accountId, instanceId) {
    return getRelayerFee(relayer, accountId, instanceId);
  }
  async sendWithdraw(relayer, payload) {
    return sendWithdraw(relayer, payload);
  }
  async prepareWithdraw(fee, note, relayer, recipient, currencyContract, logger, allowlistTreeCache, commitmentsTreeCache) {
    if (!this.graphqlUrl) {
    ...
    const { publicArgs } = await prepareWithdraw(
      this.nodeUrl,
      currencyContract,
      fee,
      note,
...
// src/services/hideyourcash/service.ts
var HideyourCash = class extends Actions {
  network;
  nodeUrl;
  contract;
  graphqlUrl;
  constructor(network, nodeUrl, contract, graphqlUrl, verifierUrl = "./verifier.wasm", circuitUrl = "./circuit.zkey") {
...
// src/views/ticket.ts
var viewWasNullifierSpent = async (nodeUrl, ticket) => {
...
// src/views/account.ts
var viewIsInAllowlist = async (rpcUrl, contract, accountId) => {
  return await viewFunction(rpcUrl, contract, "view_is_in_allowlist", {
    account_id: accountId
  });
};
var viewAccountHash = async (rpcUrl, contract, accountId) => {
  return await viewFunction(rpcUrl, contract, "view_account_hash", {
    account_id: accountId
  });
};
var viewAccountBalance = async (rpcUrl, contract, accountId) => {
  return await viewFunction(rpcUrl, contract, "ft_balance_of", {
    account_id: accountId
  });
};

// src/views/relayer.ts
var viewRelayerHash = async (rpcUrl, contract, relayer) => {
  return await viewFunction(rpcUrl, contract, "view_account_hash", {
    account_id: relayer.account
  });
};

// src/views/fungible-token.ts
var viewFungibleTokenMetadata = async (rpcUrl, contract) => {
  return await viewFunction(rpcUrl, contract, "ft_metadata");
};

// src/views/currencies.ts
var viewAllCurrencies = async (rpcUrl, contract) => {
...
var viewCurrencyContracts = async (rpcUrl, contract, currency) => {
  return await viewFunction(rpcUrl, contract, "view_currency_contracts", {
    type: currency.type,
    currency
  });
};
var viewIsContractAllowed = async (rpcUrl, contract, accountId) => {
  return await viewFunction(rpcUrl, contract, "view_is_contract_allowed", {
    account_id: accountId
  });
};
var viewIsAllowlistRootValid = async (rpcUrl, contract, root) => {
  return await viewFunction(rpcUrl, contract, "view_is_allowlist_root_valid", {
    root
  });
};

// src/views/snark-proof.ts
var viewIsWithdrawValid = async (rpcUrl, contract, currencyContract, payload) => {
